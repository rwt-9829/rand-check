...
class TestFeatures:
    ...
    def test_feature_vector_as_dict(self):
        fv = compute_features([0, 1, 0, 1, 0, 0], baseline_prob=0.50)
        d = fv.as_dict()
    ...
    def test_short_sequence_doesnt_crash(self):
        fv = compute_features([0], baseline_prob=0.50)
        assert fv.n_observations == 1

    def test_detection_summary_string(self):
        seq = [0, 1] * 20
        fv = compute_features(seq, baseline_prob=0.50)
        summary = fv.detection_summary(0.50)
    ...
    def test_detection_summary_detailed(self):
        seq = [0, 1] * 20
        fv = compute_features(seq, baseline_prob=0.50)
        summary = fv.detection_summary(0.50, detailed=True)
