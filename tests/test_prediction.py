...
from rand_check.models import Action, HandResult, Position, DecisionPackage
...
class TestPredictionEngine:
    ...
    def test_hands_processed_increments(self):
        engine = PredictionEngine()
        engine.process_action(0)
        engine.process_action(1)
        assert engine.hands_processed == 2
    ...
    def test_process_sequence(self):
        ...
        assert pkg.hands_observed == 8

    def test_process_hand_with_position(self):
        engine = PredictionEngine()
        hand = HandResult(
        ...
        pkg = engine.process_hand(hand)
        assert pkg.gto_3bet_prob == 0.14  # BTN vs CO at 100bb

    def test_edge_calculation(self):
        engine = PredictionEngine(default_gto_prob=0.25)
        # Feed lots of 3bets → predicted prob should go up → positive edge
        for _ in range(30):
    ...
    def test_reset(self):
        ...
        for _ in range(20):
            engine.process_action(0)
        engine.reset()
        assert engine.hands_processed == 0
        assert engine.history == []

    def test_exploitation_suggestion_present(self):
        engine = PredictionEngine()
        for _ in range(30):
            engine.process_action(1)
        pkg = engine.process_action(1)
        assert pkg.exploitation_suggestion != ""

    def test_hands_to_reliable_decreases(self):
        engine = PredictionEngine()
        pkg1 = engine.process_action(0)
        for _ in range(39):
            pkg = engine.process_action(0)
        assert pkg.hands_to_reliable == 0
