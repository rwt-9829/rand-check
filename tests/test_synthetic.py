...
class TestSyntheticGenerators:
    ...
    def test_iid_bernoulli_length(self):
        seq = generate_iid_bernoulli(100, 0.25)
    ...
    def test_iid_bernoulli_frequency(self):
        ...
        seq = generate_iid_bernoulli(10000, 0.25, rng)
        freq = sum(seq.sequence) / len(seq.sequence)
        assert abs(freq - 0.25) < 0.03

    def test_markov_alternation(self):
        seq = generate_markov_alternation(100, 0.25, 0.60)
        assert len(seq.sequence) == 100
        assert seq.is_human
    ...
    def test_markov_alternation_rate(self):
        ...
        rng = np.random.default_rng(42)
        seq = generate_markov_alternation(1000, 0.25, 0.65, rng)
    ...
    def test_gamblers_fallacy(self):
        seq = generate_gamblers_fallacy(100, 0.25)
        assert len(seq.sequence) == 100
        assert seq.is_human

    def test_counter_model(self):
        seq = generate_counter_model(100, 0.25)
        assert len(seq.sequence) == 100
        assert seq.is_human
    ...
    def test_counter_model_frequency_stable(self):
        ...
        rng = np.random.default_rng(42)
        seq = generate_counter_model(500, 0.25, correction_strength=3.0, rng=rng)
        freq = sum(seq.sequence) / len(seq.sequence)
        assert abs(freq - 0.25) < 0.10

    def test_run_averse(self):
        seq = generate_run_averse(100, 0.25, max_consecutive=2)
        assert len(seq.sequence) == 100
        # Check no runs > 2
        run_len = 1
    ...
    def test_mixture(self):
        seq = generate_mixture(100, 0.25)
        assert len(seq.sequence) == 100
        assert seq.is_human

    def test_changepoint(self):
        seq = generate_changepoint(100, 0.25, changepoint_at=50)
        assert len(seq.sequence) == 100
        assert seq.is_human
    ...
    def test_deterministic_with_seed(self):
        seq1 = generate_iid_bernoulli(50, 0.25, np.random.default_rng(42))
        seq2 = generate_iid_bernoulli(50, 0.25, np.random.default_rng(42))
